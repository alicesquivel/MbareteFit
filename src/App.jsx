...
export default function App() {
    ...
    (async () => {
      try {
        if (typeof __initial_auth_token !== "undefined") {
          await signInWithCustomToken(rtdbAuth, __initial_auth_token);
        } else {
          await signInAnonymously(rtdbAuth);
        }
        setUserId(rtdbAuth.currentUser.uid);
      } catch (error) {
  ...
  const saveCurrentProfileData = useCallback(async () => {
    ...
    const appId = typeof __app_id !== "undefined" ? __app_id : "default-app-id";
    const userRef = ref(db, `artifacts/${appId}/users/${userId}`);
    await set(userRef, updatedAppData);
  }, [
    activeProfile,
    appData,
    db,
  ...
  const stats = useMemo(() => {
    ...
    const weeklyAvg =
      weeklyData.length > 0
        ? weeklyData.reduce(
            (s, c) => s + (typeof c.weight === "number" ? c.weight : 0),
            0
          ) / weeklyData.length
        : null;
    const monthlyAvg =
    ...
    return {
      current:
        typeof latestEntry.weight === "number" ? latestEntry.weight : null,
  ...
  const handleLogWeight = () => {
    ...
    const previousLowest =
      weightData.length > 0
        ? Math.min(
            ...weightData.map((d) =>
              typeof d.weight === "number" ? d.weight : Infinity
            )
          )
        : Infinity;
  ...
  const handleCreateProfile = async () => {
    ...
    if (appData.profiles[profileInput.trim()])
  ...
  const chartData = useMemo(() => {
    ...
    if (typeof goalWeight === "number" && goalWeight && dataPoints.length > 0) {
      datasets.push({
        label: `Goal (${units})`,
        data: [
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md">
            ...
            <div className="space-y-3 text-sm">
              ...
              <StatItem
                label="BMI"
                value={
                  typeof stats.bmi === "number" && !isNaN(stats.bmi)
                    ? stats.bmi
                    : "--"
                }
              ...
              <StatItem
                label="7-Day Avg"
                value={formatWeight(stats.weeklyAvg, units)}
                flash={flashStats}
              />
              <StatItem
                label="30-Day Avg"
                value={formatWeight(stats.monthlyAvg, units)}
                flash={flashStats}
              />
              <hr className="my-2" />
              <StatItem
                label="Lowest"
                value={formatWeight(stats.lowest, units)}
              />
              <StatItem
                label="Highest"
